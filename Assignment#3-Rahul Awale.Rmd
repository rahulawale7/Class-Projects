---
title: 'Assignment #3- Rahul Awale'
...
```
```{r assignment, warning=FALSE, message=FALSE}
...
#No, the variable seems to be non-stationary. Based on the plot, the mean value of the time series is not constant and the variance changes over time. 
...
#   Label this time series as cpi.pred
...
#->The CPI will intrease in the next 12 months.
...
#   Based on the signature of these graphs, does the variable appear
#   stationary? Explain
#->The signature of the graphs seem to be non-stationary as the variance chages over time. In addition, the acf and pacf plots show seasonality trends. 
...
#   How accurate is the model? 
#-> The model is 14.14% lower than the predicted rate.
...
#-> Based on these graphs, we can see auto correlation in both residual and squared values. This means there is volatility so a garch model should be fitted to the residual series.
...
#the inflation volatility will be around 0.10.
